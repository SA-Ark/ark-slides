 ...
 */
export async function createExcelWorksheet (chartObject: ISlideRelChart, zip: Record<string, Uint8Array>): Promise<string> {
	const data = chartObject.data

	return await new Promise((resolve, reject) => {
		const excelFiles: Record<string, Uint8Array> = {}
		const addExcelFile = (path: string, content: string): void => {
		...
		try {
			...
			zip['ppt/charts/_rels/' + chartObject.fileName + '.rels'] = strToU8(
				'<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' +
				'<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">' +
